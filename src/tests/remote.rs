...
#[test]
fn remote_menu() {
    snapshot!(setup_clone!(), "M");
}

#[test]
fn add_remote_name_prompt() {
    snapshot!(setup_clone!(), "Ma");
}

#[test]
fn add_remote_url_prompt() {
    snapshot!(setup_clone!(), "Matest<enter>");
}

#[test]
fn add_remote() {
    snapshot!(setup_clone!(), "Matest<enter>localhost<enter>");
...
#[test]
fn rename_remote() {
    snapshot!(setup_clone!(), "Mrorigin<enter>origin2<enter>");
}

#[test]
fn remove_remote() {
    snapshot!(setup_clone!(), "MKorigin<enter>y");
}
...
fn get_upstream_components_of_feature_branch() {
    let ctx = repo_setup_clone!();
    let repo = ctx.local_repo;
    run(&ctx.dir, &["git", "checkout", "-b", "feature-branch"]);
    run(&ctx.dir, &["git", "branch", "--set-upstream-to", "main"]);
...
fn get_push_remote_with_default() {
    ...
    run(&ctx.dir, &["git", "config", "remote.pushDefault", "origin"]);
